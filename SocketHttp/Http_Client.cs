...
namespace SocketHttp
{
    ...
    public class Http_Client
    {
        ...
        private MemoryStream init(string url, string datainfo = null, string method = "get",  string charset = "UTF-8", string referer = null)
        {
            ...
            if(referer!=null)
            {
                sb.Append("Accept: "+referer+"\r\n");
                sb.Append("origin: https://cl.k6j9.icu\r\n");
            ...
            if(status_code!=200)
            {
                Console.WriteLine("状态码：【{0}】 , [ url ]：{1}", status_code, url);
            }
            switch (status_code)
            {
        ...
        private void SetHeaders(string headText)
        {
            ...
            foreach (string head in headers)
            {
                ...
                {
                    this.Cookies = this.Cookies ?? new Dictionary<string, string>();// new List<string>();
                    ...
                    //}
                    var key = tCookie.Split('=')[0];
                    var value = tCookie.Split('=')[1];
            ...
            do
            {
                for (int i = startIndex; i < readLength; i++)
                {
                    ...
                    if (c == '\n')
                    {
                        ...
                        {
                            Console.WriteLine("Maybe exists 'chunk-ext' field." + e.Message);
                        }
                        break;
                    }
                    tChars.Add(c);
